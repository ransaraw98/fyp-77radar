...
#define TASK3_PRIORITY (configMAX_PRIORITIES - 3)
...
xQueueHandle queue1to2;
xQueueHandle queue2to3;
xQueueHandle queue3to1;
...
#define NUMBER_OF_CYCLIC_TRANSFERS	10000000
...
// Task functions
//Read the buffer from header and transmit the data, put the received data in the queue.
...
void task2(void *pvParameters)
{
	u8 * tsk2recvBuf;
	BaseType_t status;
	unsigned long miscount = 0;
	while(1)
    {
    status =  xQueueReceive(queue1to2,tsk2recvBuf,0);
     if (status == pdTRUE){ xil_printf("received data packet in task 2 \r\n"); }
	else{
		miscount = miscount + 1;
	}
    }
